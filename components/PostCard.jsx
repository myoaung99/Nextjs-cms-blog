...
const PostCard = ({ post }) => {
  ...
  return (
    <div
      className=" shadow-lg rounded-lg p-0 lg:p-8 md:pb-12 mb-8"
    ...
    >
      <div className="rounded-lg relative overflow-hidden shadow-md pb-10 md:pb-16 mb-6">
        ...
        <Link href={`post/${post.slug}`}>
          <h2
            className="transition duration-700 text-center text-3xl font-semibold hover:cursor-pointer
                        hover:text-rose-700 my-5 px-4"
          >
            {post.title}
          </h2>
        </Link>
        ...
        <div className="text-center mt-8">
          <Link href={`/post/${post.slug}`}>
            <button
              className="transition duration-500 cursor-pointer hover:-translate-y-1 hover:bg-rose-700
                        bg-rose-600 text-white py-2 px-4 rounded-full"
            >
              Continue reading
            </button>
          </Link>
