...
const CommentForm = ({ slug }) => {
  ...
  return (
    <div
      className="rounded-lg shadow-lg mb-8 p-8 pb-12"
      style={{ backgroundColor: "rgb(208 250 250)" }}
    >
      <h2 className="font-semibold text-xl mb-3">Leave a comment</h2>
      ...
      <form className="pt-8" onSubmit={formSubmitHandler}>
        ...
        <div className="flex items-center">
          ...
          <label
            htmlFor="storeData"
            className="text-xs text-gray-700 ml-2 cursor-pointer hover:text-pink-700"
          >
            Remember me
          </label>
        </div>
        {error && (
          <p className="text-red-700 text-xs pl-4 mt-2 ">
            All fields are required !
          </p>
        )}

        <div className="mt-6 flex items-center justify-between">
          <button className="bg-pink-700 px-4 py-1 rounded-3xl text-white text-sm cursor-pointer hover:bg-indigo-700 transition duration-500 ease  ">
            Post Comment
          </button>
          {showSuccess && (
            <p className="text-green-700 text-xs">
              Sending your comment to author for review.
            </p>
          )}
        </div>
      </form>
